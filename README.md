...
版本：1.0  
整理日期：2026-08-01  
状态：里程碑 0–9 已实现；里程碑 10（缠论算法集扩展）已完成至 10V，目录覆盖审计待进行
...
10V 已发布 `unified_risk_execution_overlay` 并对齐 `ALG-RISK-001`。该定义以独立 `risk_filter` 进入正式回测和训练—验证 Study，默认启用无杠杆、持仓/板块/事件风险、压力损失、日损/回撤、成交量参与率、数据状态和信号年龄限制，只能批准、减量或阻断策略订单。数据 revision、陈旧/缺口或风险预算触发持久熔断，熔断后仍允许可执行的减仓和平仓。完整风险事实参与签名并写入 run/study manifest；每次决定落入 Parquet/正式日志/summary，并作为因果对象在对象树和主图展示。下一步不是宣布整个 27 项目录完成，而是审计所有目录 ID 的实现映射，尤其是仍需真实递归级别对象图的 `ALG-STR-006` 与 `ALG-STR-010`。
