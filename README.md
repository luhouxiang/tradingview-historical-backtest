...
版本：1.0  
整理日期：2026-08-01  
状态：里程碑 0–9 已实现；里程碑 10 已完成至 10AB；里程碑 11A-H、11B、11C、11D 已实现；里程碑 12A–12E、13A–13I、14A–14E 已实现
...
`before_bar_index`，从历史定位窗口向右查询使用 `after_bar_index`，游标查询默认读取
1500 根、单次最多 5000 根。响应采用列式 JSON，包含返回范围、
`has_more_before`、`has_more_after` 和校验和；不同 revision 返回 409。
