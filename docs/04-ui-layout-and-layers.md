...
- 填充：Region Fill，透明度建议 8%–15%。
- 边框与标签：Strategy Overlay。
- 缠论笔：高于均线和中枢边框。
...
普通指标只开放线型输出；缠论按分型、笔、段和中枢四个语义输出配置，并与类别可见性同步。段默认使用黄色线。
