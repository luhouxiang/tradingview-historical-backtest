...
“回测”入口打开独立浏览器工作区，并用 `dataset_id + data_revision` 与发起时的 K 线窗口绑定。
独立页的参数/摘要与交易明细是两个各自可滚动的纵向子页面；交易明细默认至少占可用高度的一半，
中间分隔条可向上拖动扩大明细区，双击恢复一半。双击交易行通过同源页面通信把原 K 线窗口定位到
该笔入场 K 线并高亮成交锚点；数据 revision 不匹配时必须拒绝联动。
