...
滑点、净盈亏和结构归因；每笔完整交易在界面投影为 `-01` 开仓和 `-02` 平仓两条成交记录，双击时定位各自的 K 线。交易表中的“成交原因”必须以该条记录对应的 `entry_signal_id/exit_signal_id` 关联同一正式 run 的因果 `trade_signal.reason_code`；通用结构快照归因只作为入场时市场上下文，不是交易触发原因。整笔盈亏和费用只在平仓行结算，开仓行不重复计数。跨窗口消息不能改变数据事实，
