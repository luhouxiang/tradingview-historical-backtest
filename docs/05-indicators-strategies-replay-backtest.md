...
- `docs/structures.py`、`docs/multilevel.py`、`docs/engineering.py` 是首版参考算法。
- 顶分型与底分型之间至少满足配置的 K 线间距；默认至少 5 根原始 K 线，并由算法版本解释。
- 分型默认左右各 2 根严格极值，且右侧窗口之后还需一根独立 K 线才封存。
- 中枢由三笔严格重叠建立，随后记录确认、延伸、离开状态和离开方向。
- 按 C-015 不实现也不输出线段。
