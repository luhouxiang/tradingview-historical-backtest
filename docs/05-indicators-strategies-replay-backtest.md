...
回测 UI 必须遍历 `trades` 的分页游标，不能只展示首 100 笔。独立回测工作区固定绑定发起时的
`dataset_id + data_revision`，交易表显示入场/出场时间、K 线索引、价格、手数、毛盈亏、手续费、
滑点、净盈亏和结构归因；每笔完整交易在界面投影为 `-01` 开仓和 `-02` 平仓两条成交记录，双击时定位各自的 K 线。交易表中的“成交原因”必须以该条记录对应的 `entry_signal_id/exit_signal_id` 关联同一正式 run 的因果 `trade_signal.reason_code`；三买开仓还应投影该事件冻结的来源中枢、离开段、首次回试段、边界差值、确认时点和成交量门槛，不得由 Vue 从当前图形重新推导。通用结构快照归因只作为入场时市场上下文，不是交易触发原因。整笔盈亏和费用只在平仓行结算，开仓行不重复计数。跨窗口消息不能改变数据事实，
revision 不一致时不得定位或加载运行结果。
